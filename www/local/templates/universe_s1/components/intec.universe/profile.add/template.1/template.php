            ...
            <?php if ($USER->IsAuthorized()) { ?>
                <form class="profile-add-form" method="post" action="" enctype="multipart/form-data" data-role="profile.form">
                    ...
                    <div class="profile-add-form-field-group">
                        <div class="profile-add-form-field-title profile-add-form-field-title-required" style="display: none;">
                            <?= Loc::getMessage('C_PROFILE_ADD_TEMPLATE_DEFAULT_PERSON_TYPE_TITLE') ?>
                        </div>
                        <div class="profile-add-form-block">
                            <div class="intec-grid intec-grid-wrap intec-grid-a-h-start intec-grid-a-v-start intec-grid-i-8" style="display: none;">						
                        ...
                        <?php if (!empty($arGroup['PROPS'])) { ?>
                            ...
                            <div class="profile-add-form-field-group">
                                <?php foreach($arGroup['PROPS'] as $arProperty) { ?>
                                    ...
                                    $arCurrentValue = is_array($arResult['ORDER_PROPS_VALUES'][$sPropertyName]) ? 
									$arResult['ORDER_PROPS_VALUES'][$sPropertyName] : 
									[$arResult['ORDER_PROPS_VALUES'][$sPropertyName]];
                                    ?>
                                    <div class="profile-add-form-block">
										<?php if ($arProperty["HIDDEN"] != "Y") {?>
                                        ...
                                        <div class="profile-add-form-field" data-role="property.row">
                                            ...
                                            <?php } else if ($arProperty['TYPE'] == 'TEXT') { ?>	
												<?php if (isset($arProperty["HIDDEN"]) && $arProperty["HIDDEN"] == "Y") {?>
													<input type="hidden" name="<?=$sPropertyName?>" value="<?=$arProperty["VALUE"]?>">
												<?php } else {?>
													<?php foreach ($arCurrentValue as $key => $sValue) { ?>
