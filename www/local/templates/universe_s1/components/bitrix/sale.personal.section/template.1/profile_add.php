...
$typeClient = intec\eklectika\advertising_agent\Client::getStatus();
if (!isset($_REQUEST["PERSON_ID"])) {
	if ($typeClient == "fiz") {
		$url = $_SERVER["SCRIPT_URL"]."?PERSON_ID=1";
	} else {
		$url = $_SERVER["SCRIPT_URL"]."?PERSON_ID=2";
	}
	header("Location:" . $url); 
}
if (isset($_REQUEST["PERSON_ID"])) {
	if ($typeClient == "fiz" && $_REQUEST["PERSON_ID"] != 1) {
		header("Location:" . $_SERVER["SCRIPT_URL"]."?PERSON_ID=1"); 
	}
	if (($typeClient == "jur" || $typeClient == "agent") && $_REQUEST["PERSON_ID"] != 2) {
		header("Location:" . $_SERVER["SCRIPT_URL"]."?PERSON_ID=2"); 
	}
