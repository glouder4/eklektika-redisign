...
<script type="text/javascript">
    template.load(function (data) {
		...
		var createReserve = $('.basket-create-reserve', root);
        var items;
        var quickItemsId = [];
		...
		save.on('click', function() {
			let self = $(this);
			$.ajax({
				url: "/personal/basket/save.php",
			}).done(function() {
				alert("Корзина сохранена, ее можно увидеть в личном кабинете");
				self.hide();
			});
		...
		createReserve.on('click', function() {
			...
			if (conf) {
				$.ajax({
					url: "/personal/basket/create_reserve.php",
				}).done(function() {
