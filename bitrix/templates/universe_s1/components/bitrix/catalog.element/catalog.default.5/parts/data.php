...
$hData = function (&$arItem, $bOffer = false) use (&$arResult, &$bBase, &$bLite) {
    if ($arResult["PROPERTIES"]["MINIMALNYY_TIRAZH"]["VALUE"] && $arItem['CATALOG_QUANTITY'] < $arResult["PROPERTIES"]["MINIMALNYY_TIRAZH"]["VALUE"]) {
    ...
    $arData = [
        ...
        'quantity' => [
            'value' => !Type::toBoolean($arItem['CAN_BUY']) ? 0 : Type::toFloat($arItem['CATALOG_QUANTITY']),
