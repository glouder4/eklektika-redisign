...
| [`local/classes/requires.php`](../../local/classes/requires.php) | `require_once` для **`/local/modules/<module_id>/include.php`** в порядке **`eklektika.b24.rest` → `eklektika.company` → `eklektika.catalog.pricing` → `eklektika.site` → `eklektika.catalog.import` → `eklektika.orders.applications` → `eklektika.b24.usersync`**; при отсутствии `include.php` — `E_USER_WARNING` (fail-safe, без фатала); затем **`CatalogPriceFloor::markCompositeNonCacheableForAuthorizedCatalog()`**. Поиск **`BeforeIndex`** регистрируется из **`eklektika.site/include.php`**. |
