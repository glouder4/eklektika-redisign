...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	protected function initProperties(Order $order, $isPersonTypeChanged)
	{
		...
		if ($haveProfileId)
		{
			$profileProperties = (new \OnlineService\Site\Company)->getProfileValues((int)$this->arUserResult['PROFILE_ID']);// Sale\OrderUserProperties::getProfileValues((int)$this->arUserResult['PROFILE_ID']);
		}
    ...
    private function getSelectedCompanyFields($id){
        return (new \OnlineService\Site\Company)->getCompany($id);
    }
	...
	protected function getJsDataResult()
	{
        ...
        if( count($this->customDataFieldIds) > 0 ){
            $checkedProfiles = array_filter($result['USER_PROFILES'], function($profile) {
                return isset($profile['CHECKED']) && $profile['CHECKED'] === 'Y';
            });

            $selectedId = !empty($checkedProfiles) ? key($checkedProfiles) : null;

            $companyFields = $this->getSelectedCompanyFields($selectedId);
            ...
            foreach ($arr['properties'] as $index => $propertyData){
                if( in_array((int)$propertyData['ID'],$this->customDataFieldIds) ){
                    ...
                    elseif( $propertyData['ID'] == 12 ){
                        $arr['properties'][$index]['VALUE'][0] = $user['NAME'].' '.$user['LAST_NAME'];
                    }
                    elseif( $propertyData['ID'] == 13 ){
                        $arr['properties'][$index]['VALUE'][0] = $user['EMAIL'];
                    }
                    elseif( $propertyData['ID'] == 14 ){
                        $arr['properties'][$index]['VALUE'][0] = $user['PERSONAL_PHONE'];
                    }

                    $arr['properties'][$index]['DISABLED'] = true;
                }
            }
        }
