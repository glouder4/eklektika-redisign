	...
	else // if there's no any errors - create user
	{
		...
		switch($arResult["VALUES"]["UF_TYPE"]) {
			case "4":
				...
				unset($resultValues["UF_KPP"]);
				unset($resultValues["UF_NAME_COMPANY"]);
				unset($resultValues["UF_JUR_ADDRESS"]);
				unset($resultValues["UF_SPERE"]);
				unset($resultValues["UF_SITE"]);
				unset($resultValues["UF_REQ"]);
				unset($resultValues["UF_REQ"]);
			break;
			case "5" :
			break;
		...
		if (intval($ID) > 0)
		{
			...
			if ($arResult["VALUES"]["UF_TYPE"] == "4" && $ID) {
				$fio = $arResult["VALUES"]["LAST_NAME"]." ".
						$arResult["VALUES"]["NAME"].
						(isset($arResult["VALUES"]["SECOND_NAME"])?' '.$arResult["VALUES"]["SECOND_NAME"]:'');
				// добавить новый профиль пользователю
