...
## Цель подзадачи
Устранить рассинхрон в описании bootstrap-порядка модулей между документацией и фактическим `local/modules/bootstrap.php`.

## Описание работ
1. Считать фактический порядок `Loader::includeModule` из `local/modules/bootstrap.php` как единственный источник истины.
...
## Технические детали
- Компоненты/модули:
  - bootstrap `local/modules/bootstrap.php`
...
## Критерии приёмки
- [ ] В `company_system.md` bootstrap-цепочка совпадает с `local/modules/bootstrap.php`
...
- Unit/интеграционные проверки:
  - нет
- Ручной сценарий:
  - открыть `local/modules/bootstrap.php`, сверить порядок модулей с каждым обновлённым документом
