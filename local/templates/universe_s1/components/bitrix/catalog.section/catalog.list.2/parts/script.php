            ...
            root.update = function () {
                ...
                items.each(function () {
                    ...
                    item.offers = new app.classes.catalog.Offers({
                        'properties': properties.length !== 0 ? offerProps : properties,
                        'list': data.offers
                    });
                    ...
                    item.update = function () {
                        ...
                        _.each(prices, function(price) {
							if (price != null) {   
								...
								item.prices[price['code']].attr('data-discount', price.discount.use ? 'true' : 'false');
								item.prices[price['code']].attr('data-measure', price.discount.measure !== null ? 'true' : 'false');
								item.prices[price['code']].attr('data-range', entity.price.range ? 'true' : 'false');
								item.prices[price['code']].measure.text(price.discount.measure);
								item.prices[price['code']].base.html(price.base.display);
								item.prices[price['code']].discount.html(price.discount.display);
								item.prices[price['code']].percent.text('-' + price.discount.percent + '%');
								if (price.minPrice) {
