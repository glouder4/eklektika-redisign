...
if (!isset($_GET["ORDER_ID"]) || isset($_GET["ORDER_ID"]) && $_GET["ORDER_ID"] <> '') {
    if ($arParams['MIN_SUMM'] && $arParams['MIN_SUMM'] > $arResult['JS_DATA']['TOTAL']['PRICE_WITHOUT_DISCOUNT_VALUE']) {
...
<div id="<?= $sTemplateId ?>" class="ns-bitrix c-sale-order-ajax c-sale-order-ajax-simple-1">
    <div class="sale-order-ajax-wrapper intec-content">
        <div class="sale-order-ajax-wrapper-2 intec-content-wrapper">
            ...
            <?php } ?>
			<script>
				let typeClient = '<?=intec\eklectika\advertising_agent\Client::getStatus()?>';
				if (typeClient == 'agent' || typeClient == 'jur') {
					window.personType = 2;
				} else {
					window.personType = 1;
				}
