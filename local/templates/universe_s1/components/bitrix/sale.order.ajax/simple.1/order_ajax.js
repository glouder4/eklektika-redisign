...
// console.log(action);
			...
			{
				let newData = this.getData(action, actionData);			
					console.log(this.result);
					if (this.result.PERSON_TYPE.length) {
						newData.order.PERSON_TYPE = this.result.PERSON_TYPE[0].ID;
					} else {
						newData.order.PERSON_TYPE = window.personType;
					}
				BX.ajax({
					method: 'POST',
					dataType: 'json',
				...
				nodes.push(BX.create('DIV', {
					...
					children: [
                        ...
                        BX.create('A', {
                            props: {
								className: 'intec-ui intec-ui-control-button intec-ui-mod-round-3 intec-ui-scheme-current intec-ui-size-2',
								href: '/personal/profile/registration.php',
								target: '_blank'
							},
