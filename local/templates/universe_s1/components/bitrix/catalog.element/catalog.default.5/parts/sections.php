...
<div class="service-description-list_items">
    ...
    $propertyIds = [
        'PLOTNOST_MATERIALA_G_M2',
        'DATIROVKA_BLOKA',
        'TIP_MEKHANIZMA_ZONTA',
        'FORMAT_OBLOZHKI',
        'OBYEM_DANNYKH_EMKOST',
        'VID_BLOKA',
        'TIP_OBLOZHKI',
        'TIP_SKLADYVANIYA_ZONTA',
        'TIP_RUCHKI_ZONTA'
    ];
    
    foreach ($propertyIds as $propertyCode) {
        if (!empty($arResult['PROPERTIES'][$propertyCode]['VALUE'])) {
            renderPropertyItem($arResult['PROPERTIES'][$propertyCode], '#97e5b7');
        }
    }
    ?>
</div>
<?/* переделано в helpers.php
if (!empty($arResult['PROPERTIES']['DESCRIPTION_PLUS']['VALUE'])) {
    //$elementId = $arResult['PROPERTIES']['DESCRIPTION_PLUS']['VALUE'][0];
    foreach ($arResult['PROPERTIES']['DESCRIPTION_PLUS']['VALUE'] as $elementId){
...
<div class="warning-text-cust">
    Компания ЙО!каталог оставляет за собой право без предварительных уведомлений менять технические параметры и потребительские характеристики представленных товаров и их упаковки
</div>