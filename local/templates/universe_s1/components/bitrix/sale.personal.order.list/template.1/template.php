        ...
        <div class="sale-personal-order-list-wrapper-2 intec-content-wrapper">
                        ...
                        <div class="sale-personal-order-list-item" data-role="item">
                            <div class="intec-grid intec-grid-wrap intec-grid-a-v-center intec-grid-i-15">
                                ...
                                <div class="sale-personal-order-list-item-wrap intec-grid-item intec-grid intec-grid-wrap intec-grid-a-h-start intec-grid-a-h-768-around intec-grid-a-v-center" data-code="button">
                                    ...
                                    <?php 
									if ($arOrder['ORDER']['CANCELED'] !== 'Y' && $arOrder['ORDER']['PAYED'] !== 'Y' 
									&& $arOrder['PROPERTIES']['REQUEST_TO_RESERVE'] !== "Y") { ?>
                                    ...
                                    <?php } ?>
									<?php if ($arOrder["PROPERTIES"]["REQUEST_TO_RESERVE"] == "Y" 
										&& $arOrder["PROPERTIES"]["RESERVE_SUCCESS"] == "Y" 
										&& $arOrder["PROPERTIES"]["IN_WORK"] != "Y") {?>
										<?= Html::a("Взять в работу", '', [
											'data-id' => $arOrder['ORDER']['ID'],
                                            'class' => [
												'sale-personal-in-work',
                                                'sale-personal-order-list-item-text',
                                                'intec-cl-text',
                                                'intec-ui' => [
                                                    '',
            ...
            <script>
                template.load(function (data) {
					...
					$(".sale-personal-in-work").click(function(e) {
						e.preventDefault();
						let conf = confirm("Вы действительно хотите отправить резерв в работу?");
						if (conf) {
							let idOrder = $(this).attr("data-id");
							$.ajax({
								url: "/personal/profile/ajax.php?action=add_in_work&order_id="+idOrder
							}).done(function() {
								alert("Запрос на взятие резерва в работу отправлен");
								 location.reload();
							});
						}
						return false;
